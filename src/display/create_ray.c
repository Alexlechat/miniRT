...
/*   Updated: 2025/10/11 11:29:46 by anpicard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_camera_basis	calculate_camera_basis(t_vector orientation)
{
	...
	// Normaliser l'orientation (direction forward)
	basis.forward = normalize(orientation);
	
	// Définir un vecteur "up" mondial
	// Si la caméra pointe vers le haut ou le bas, utiliser un vecteur différent
	...
	// Calculer le vecteur right (produit vectoriel)
	basis.right = cross_product(world_up, basis.forward);
	basis.right = normalize(basis.right);
	
	// Calculer le vecteur up (produit vectoriel)
...
int create_ray(t_display *display, int pixel_x, int pixel_y)
{
    ...
    if (hit.hit == true)
    {
        ...
        // Calcul de l'ambient lighting (réduit pour être subtil)
        ambient_r = (hit.color.r / 255.0) * (display->ambient.color.r / 255.0) * display->ambient.brightness * 255.0 * 0.3;
        ambient_g = (hit.color.g / 255.0) * (display->ambient.color.g / 255.0) * display->ambient.brightness * 255.0 * 0.3;
        ambient_b = (hit.color.b / 255.0) * (display->ambient.color.b / 255.0) * display->ambient.brightness * 255.0 * 0.3;
        
        // Calcul du diffuse lighting
        is_lit = reflection(display, hit.coordinate, &hit);
        if (is_lit == true && hit.angle_deg <= 90)
        {
            normalized_angle = 1.0 - (hit.angle_deg / 90);
            diffuse_r = hit.color.r * normalized_angle * display->light.brightness;
            diffuse_g = hit.color.g * normalized_angle * display->light.brightness;
            diffuse_b = hit.color.b * normalized_angle * display->light.brightness;
        }
        else
        {
            diffuse_r = 0;
        ...
        // Combinaison ambient + diffuse (clamping à 255)
        hit.color.r = (ambient_r + diffuse_r > 255) ? 255 : (unsigned char)(ambient_r + diffuse_r);
        hit.color.g = (ambient_g + diffuse_g > 255) ? 255 : (unsigned char)(ambient_g + diffuse_g);
        hit.color.b = (ambient_b + diffuse_b > 255) ? 255 : (unsigned char)(ambient_b + diffuse_b);
    }   
